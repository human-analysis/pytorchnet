...
class Residual(nn.Module):
    def __init__(self, inplanes, planes, stride=1):
        ...
        self.conv3 = nn.Conv2d(self.planes,self.inplanes,kernel_size=1,stride=1,padding=0)
...
class HourGlassUnit(nn.Module):
    def __init__(self, depth, nfilters, nmodules=1):
        ...
        self.conv = nn.ConvTranspose2d(self.nfilters, self.nfilters, 4, 2, 1, bias=False)
...
class HourGlassStack(nn.Module):
    def __init__(self, nfilters, noutputs, nmodules=1):
        ...
        self.output = nn.Conv2d(self.nfilters,self.noutputs,kernel_size=1,stride=1,padding=0)
        self.conv1 = nn.Conv2d(self.nfilters, self.nfilters,kernel_size=1,stride=1,padding=0)
        self.conv2 = nn.Conv2d(self.noutputs, self.nfilters,kernel_size=1,stride=1,padding=0)
...
class HourGlassLast(nn.Module):
    ...
    def make_layers_(self):
        ...
        for i in range(self.nmodules):
            modules.append(Residual(self.nfilters,self.nfilters))
        modules.append(Lin(self.nfilters,self.nfilters))
        modules.append(nn.Conv2d(self.nfilters,self.noutputs,kernel_size=1,stride=1,padding=0))
        return nn.Sequential(*modules)
...
class HourGlassSingle(nn.Module):
    ...
    def make_layers_(self):
        ...
        for i in range(self.nstack-1):
            modules.append(HourGlassStack(self.nfilters, self.noutputs, self.nmodules))
        return nn.Sequential(*modules)
